...
public class InitHandlerTest {
    ...
    public InitHandlerTest() {
        ...
        when(env.readEnv((EnvironmentVariables.CERTIFICATE_ARN_ENV)))
            .thenReturn("somthing goes here");
        when(env.readEnv((EnvironmentVariables.APPLICATION_URL)))
            .thenReturn("apihello.entitydata.aws.unit.no.");
        when(env.readEnv((EnvironmentVariables.STACK_NAME)))
            .thenReturn("aut-reg-autre-88-stati-url-service-stack-test");
        when(env.readEnv(EnvironmentVariables.STAGE_NAME)).thenReturn(Stage.TEST.toString());
        when(env.readEnv(EnvironmentVariables.API_KEY_TABLE_NAME))
            .thenReturn("aut-reg-autre-88-stati-url-test-apiKeys");
